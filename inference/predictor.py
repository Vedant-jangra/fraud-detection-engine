...
class ONNXPredictor:
    ...
    def predict(self, features: FeatureVector) -> float:
        result = self.session.run(None, {self.input_name: features.vector.astype(np.float32)})
...
class FeatureExtractor:
    def __init__(self, db_pool: asyncpg.Pool):
        ...
        self.le.fit(['grocery', 'travel', 'electronics', 'restaurant', 'utility', 'crypto', 'unknown', 'XX', 'IN', 'US', 'SG', 'AE'])
        
        self.feature_names = [
            'log_amount', 'amount_rounded', 'is_micro_txn',
            'hour_of_day', 'day_of_week', 'is_weekend', 'is_night',
            'merchant_cat_enc', 'country_enc',
            'txn_count_10m', 'total_amount_10m', 'velocity_ratio',
            'amount_spike_ratio', 'unique_merchants_10m',
            'unique_countries_10m', 'txn_count_1h'
        ]
    ...
    async def get_features(self, user_id: str, amount: float, merchant_cat: str, country: str, timestamp: str) -> FeatureVector:
