...
with col1:
    ...
    with st.form("transaction_form"):
        ...
        merchant_cat = st.selectbox("Merchant Category", 
                                    ["electronics", "grocery", "travel", "restaurant", "utility", "crypto", "unknown"])
    ...
    if submitted:
        ...
        with st.spinner("Scoring transaction..."):
            try:
                ...
                response = requests.post(f"{API_URL}/v1/explain", json=payload, timeout=5)
                end_time = time.time()
                ...
                if response.status_code == 200:
                    ...
                    if "explanation" in data and "top_factors" in data["explanation"]:
                        ...
                        st.markdown(f"**Explanation Method:** {data['explanation'].get('explainability_method', 'SHAP TreeExplainer')}")
                        ...
                        for factor in factors:
                            ...
                            if "increases" in direction.lower():
                                st.error(f"⬆️ **{feature}** strongly increased fraud risk (SHAP: {shap_val:.4f})")
                            else:
                                st.success(f"⬇️ **{feature}** decreased fraud risk (SHAP: {shap_val:.4f})")
                                
                    else:
                        st.info("No detailed explanation available for this transaction.")
                ...
                st.error("Could not connect to the FastAPI Backend. Is the API running?")
            except Exception as e:
                st.error(f"An error occurred: {str(e)}")
    else:
        st.info("👈 Fill out the transaction details and click **Score Transaction** to begin.")
