...
FRAUD_ARCHETYPES = {
    ...
    'account_takeover': {'amount_range': (5000, 50000), 'velocity': 'med', 'weight': 0.40},
...
def generate_transaction(user_id: str, is_fraud: bool = False) -> dict:
    ...
    return {
        ...
        'merchant_cat': random.choice(['grocery','travel','electronics','restaurant','utility','crypto']),
        'country': random.choices(['IN','US','SG','AE'], weights=[0.70,0.15,0.10,0.05])[0],
        'device_id': fake.md5()[:16],
        'ip_address': fake.ipv4(),
        'is_fraud': int(is_fraud),
    }

def generate_dataset(n_transactions: int = 100_000, fraud_rate: float = 0.003) -> pd.DataFrame:
    ...
    records = (
        [generate_transaction(random.choice(user_pool), False) for _ in range(legit_count)]
